...
def create_trojan_stb_account(bot, message):
...
def process_username_step(message, bot):
    try:
        ...
        if username.lower() == '/cancel':
            bot.reply_to(message, "❌ *Pembuatan akun Trojan dibatalkan*", parse_mode='Markdown')
            return
...
def process_days_step(message, username, bot):
    try:
        ...
        for line in lines:
            ...
            if '#trojan' in line:  # Jika baris mengandung #trojan di mana saja
